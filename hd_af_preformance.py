...
af_block_value = file1[~file1["blocked_reason"].isnull()].value_counts()
af_block_names = af_block_value.keys().tolist()
hd_block_value = file1[~file1["anti_type"].isnull()].value_counts()
hd_block_names = hd_block_value.keys().tolist()


str1 = " \t \t %s\r\n" % ("\t".join(af_block_name))
str2 = " \t \t %s\r\n" % ("\t".join(af_block_value.tolist()))
...
with open("af_hd_result.csv","w") as f:
    ...
    for each_hd in hd_block_names:
        ...
        for each_af in af_block_names:
            join_result = sum((file1["anti_type"] == each_hd)&(file1['anti_type'] == each_af))
            mlist.append(join_result)
        str3 = "%s\t%s" % (str3,"\t".join([str(i) for i in mlist]))
    f.write(str3)
